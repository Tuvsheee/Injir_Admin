...
const PartnerList = ({ partner }: Props) => {
  const handleDelete = (id: string) => {
    if (typeof window !== undefined) {
      if (window.confirm("Та устгахдаа итгэлтэй байна уу")) {
        axios
          .delete("https://tanusoft.tanuweb.cloud/api/v1/partner/" + id)
          .then(() => {
    ...
    <>
      <div className="w-full border-b-[#e5e5e5] border-b h-20 bg-white flex items-center justify-between px-10 top-0 z-0">
        <span className="text-[#162C43] text-sm lg:text-lg">
          Хамтрагч байгууллагын жагсаалт
        </span>
        <Link
          href={"/partner/add"}
                ...
                <tr key={list._id} className="">
                  ...
                  <td>
                    <div className="flex gap-2 items-center">
                      <img
                        src={
                          "https://tanusoft.tanuweb.cloud/uploads/" + list?.logo
                        }
                        alt=""
                  ...
                  <td>
                    <div className="flex items-center gap-4">
                      <Link href={"/partner/edit/" + list._id}>
                        <Edit
                          color="orange"
                          size={20}
